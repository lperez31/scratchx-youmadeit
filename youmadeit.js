...
/*
client = new Messaging.Client("api.youmadeit.fr", 1883, "clientId");
 client.onConnectionLost = onConnectionLost;
 client.onMessageArrived = onMessageArrived;
 client.connect({onSuccess:onConnect});
...
myServer.host = "api.youmadeit.fr";
myServer.port = 1884;
...
function sendToDevice(targetDeviceName, paramName, data) {
    encodedTargetDeviceName = encodeURIComponent(targetDeviceName)
    var topic = "client/" + my_ids.apiKey + "/" + encodedTargetDeviceName;
...
function onConnect() {
  ...
  var subscribeTopic = "client/" + my_ids.apiKey + "/" + my_ids.encodedDeviceName
...
/////////////////////////////////////
    ...
    ext._shutdown = function() {
        if (client != null) {
            console.log("Disconnecting YouMadeIT");
            client.disconnect();
        }
    };
    ...
    ext.connect_youmadeit = function(apiKey, deviceName) {
        ...
        init_youmadeit(apiKey, deviceName, function() {
            // On Connect
            console.log("Connected to YouMadeIt with apiKey " + apiKey);        
            onConnect();
        });
    }

    ext.send_message = function(paramName, value, targetDeviceName) {
    ...
    ext.get_message_param_name = function() {
        return message_received_parameter_name;
    }

    ext.get_message_value = function() {
        return message_received_value;
    }
    ...
    ScratchExtensions.register('YouMadeIT extension', descriptor, ext);
})({});